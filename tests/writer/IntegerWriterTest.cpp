...
TEST(IntegerWriterTest, WriteRunLengthEncodeIntWithoutNull) {
    ...
    auto column_chunk_encoding =
        integer_column_writer->getColumnChunkEncoding();
    ...
    while (num_to_read) {
        ...
        for (int i = vector_index; i < vector_index + size; i++) {
            std::cerr << "[DEBUG READ CASE1] "
                      << int_result_vector->intVector[i] << std::endl;
...
TEST(IntegerWriterTest, DISABLED_WriteRunLengthEncodeLongWithoutNull) {
    ...
    while (num_to_read) {
        auto size = std::min(pixel_stride, num_to_read);
        long_column_reader->read(
            buffer, column_chunk_encoding, pixel_offset, size, pixel_stride,
            vector_index, long_result_vector,
            *(long_column_writer->getColumnChunkIndexPtr()), bit_mask);
        for (int i = vector_index; i < vector_index + size; i++) {
            std::cerr << "[DEBUG READ CASE1] "
                      << long_result_vector->longVector[i] << std::endl;
