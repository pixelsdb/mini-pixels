    ...
    switch(columnVector->physical_type_) {
        case PhysicalType::INT16:
            for(int i = 0; i < size; i++) {
                std::memcpy((uint8_t*)columnVector->vector + (vectorIndex + i) * sizeof(int16_t), 
                    input->getPointer() + input->getReadPos(), sizeof(int16_t));
                input->setReadPos(input->getReadPos() + sizeof(int64_t)); 
            }
            break;
        case PhysicalType::INT32:
            for(int i = 0; i < size; i++) {
                std::memcpy((uint8_t*)columnVector->vector + (vectorIndex + i) * sizeof(int32_t), 
                    input->getPointer() + input->getReadPos(), sizeof(int32_t));
                input->setReadPos(input->getReadPos() + sizeof(int64_t)); 
            }
            break;
        ...
        case PhysicalType::INT128:
            ...
            break;
        default:
            throw std::runtime_error("DecimalColumnReader: Unexpected Physical Type");
    }
}
